...
namespace Microsoft.Xna.Framework.Graphics
{
    public class Texture2D : Texture
    {
        ...
        {
            // GG EDIT added constructor
            this.graphicsDevice = graphicsDevice;
            this._width = width;
            this._height = height;
            this._format = format;
            this._mipmap = numberLevels > 1;
        ...
        private void generateOpenGLTexture()
        {
            ...
            if (texture == null) 
            {
                texture = new ESImage(new ESTexture2D(Height, Width, (uint)textureId));
            }
        }
        ...
        public Color GetPixel(int x, int y)
        {
            ...
            try {
                ...
                // GG TODO commented out for NACL, think this isn't used
                ...
                Marshal.Copy(new IntPtr(tempPtr.ToInt64() + pos), data, 0, 4);
        ...
        public static Texture2D FromFile(GraphicsDevice graphicsDevice, System.IO.Stream textureStream)
        {
            Bitmap image = (Bitmap)Bitmap.FromStream(textureStream);
			
			if (image == null)
            {
				throw new ContentLoadException("Error loading Texture2D System.IO.Stream");
			}
        ...
        public static Texture2D FromFile(GraphicsDevice graphicsDevice, System.IO.Stream textureStream, int numberBytes)
        {
            throw new NotImplementedException();
        }
        ...
        public static Texture2D FromFile(GraphicsDevice graphicsDevice, string filename)
        {
			...
			ESImage theTexture = new ESImage(image, graphicsDevice.PreferedFilter);
			Texture2D result = new Texture2D(theTexture);
			result.Name = NaCl.PLFS.Path.GetFileNameWithoutExtension(filename);
			return result;
        }
